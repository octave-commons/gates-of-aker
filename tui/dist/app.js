...
export class App {
    ...
    constructor() {
        ...
        this.wsClient = new WSClient(wsUrl, this.handleMessage.bind(this), this.handleStatusChange.bind(this));
    ...
    handleMessage(msg) {
        switch (msg.op) {
            ...
            case "tick_delta":
                if (this.state) {
                    this.state.tick = msg.data.tick ?? this.state.tick;
    ...
    setupKeybindings() {
        ...
        this.mapView.getElement().key(["up", "down", "left", "right"], (ch, key) => {
            this.handleMapNavigation(key.name);
        });
    ...
    handleMapSelection() {
        if (!this.selectedCell || !this.state)
            return;
        const [q, r] = this.selectedCell;
        const tileKey = `${q},${r}`;
