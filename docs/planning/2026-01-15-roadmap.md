...
- [[spec/2026-01-15-myth-engine.md]] - Myth engine details
- [[spec/2026-01-15-gates-of-aker-roadmap.md]] - Concise roadmap summary
- [[docs/notes/design/2026-01-15-world-schema.md]] - World schema details
...
**Done means** ✅ Completed 2026-01-17
...
**Implementation**: `backend/src/fantasia/sim/hex.clj` + `web/src/components/SimulationCanvas.tsx`
...
**Implementation**:
- `backend/src/fantasia/sim/pathing.clj` - BFS (line 5) + A* (line 33)
- `backend/src/fantasia/sim/spatial.clj` - passability checks
- `:job/build-wall` in `jobs.clj` converts ghost→wall (line 122)


### Milestone 3 — Colony job loop (RimWorld skeleton)

**Done means** 🟡 90% COMPLETE - Code complete, verification pending
...
- ✅ Job system code exists (`backend/src/fantasia/sim/jobs.clj` - 900+ lines)
  - Job creation (line 21), assignment (line 41), auto-assignment (line 84)
  - Stockpile handling (lines 146-178)
  - Job types: chop-tree, harvest-wood, harvest-fruit, harvest-grain, harvest-stone, build-house, build-structure, haul, deliver-food, build-wall, eat, sleep
  - Modular job types in `jobs/` subdirectory (chop.clj, eat.clj, haul.clj, sleep.clj, deliver_food.clj)
- ✅ Pathing algorithms exist (`backend/src/fantasia/sim/pathing.clj`)
  - BFS (line 5), A* (line 33)
- ✅ Mortality system (`backend/src/fantasia/sim/tick/mortality.clj`)
  - Death checking (starvation, health-critical)
  - Death memory creation with facets
  - Job cleanup on death
- ✅ Integration calls in `tick/core.clj`
  - `auto-generate-jobs!` (line 47)
  - `auto-assign-jobs!` (line 48)
  - `mortality/process-mortality!` (line 46)
  - `move-agent-with-job` uses pathing
- ✅ Tree system (`backend/src/fantasia/sim/tick/trees.clj`)
  - Tree spreading, fruit dropping
- ✅ Building/resource placement (`backend/src/fantasia/sim/tick/actions.clj`)
  - Wall ghosts, stockpiles, warehouses, campfires, statues, trees, wolves, bears
- ⚠️ Frontend UI exists (JobQueuePanel, AgentCard, BuildControls, BuildingPalette)
  - Agent needs not visible in UI
  - Job progress not visible in UI
- ⚠️ Logging partially implemented
  - ✅ Job creation/assignment logs exist (`[JOB:CREATE]`, `[JOB:ASSIGN]`, `[JOB:AUTO-ASSIGN]`)
  - ✅ Job completion logs exist for most job types
  - ✅ Movement logs exist (`[MOVEMENT:AGENT]`, `[MOVEMENT:RANDOM]`)
  - ❌ Missing: Pathing request logs (`[PATH:REQUEST]`, `[PATH:RESULT]`)
  - ❌ Missing: Job progress logs (`[JOB:PROGRESS]`, `[JOB:ADJACENT]`)
  - ❌ Missing: Need trigger logs (`[JOB:NEED-TRIGGER]`)

**🟡 BLOCKING ISSUE**: Incomplete observability
- Cannot verify complete job lifecycle (creation → assignment → progress → completion)
- Cannot verify pathing works end-to-end
- Cannot verify needs-driven behavior triggers correctly

**Next steps for completion**:
1. Add missing logging per `spec/2026-01-18-observability.md` (phases 1-6)
2. Run verification scenarios (chop tree → wood, eat → need restore, build wall)
3. Add agent needs visualization to AgentCard.tsx
4. Add job progress visualization to JobQueuePanel.tsx
5. Run full test suite and fix failures

**Progress**: ~18/20 SP complete (90%)

**Details**: See `docs/notes/planning/2026-01-19-milestone3-3.5-progress-review.md`


### Milestone 3.5 — Facet/Memory System (Agent Perception)

**Done means** 🟡 78% COMPLETE - Backend complete, frontend pending

*   Agents perceive world through facet-based queries
*   Memories decay over time and affect perception
*   Death creates memories with facets
*   Needs axes map to concept queries

**Implementation Status**:
- ✅ Spatial facets system (`backend/src/fantasia/sim/spatial_facets.clj` - 300+ lines)
  - Embedding cache (currently stub vectors)
  - Entity facet registry (13 M3 entity types)
  - Cosine similarity computation
  - Query function `query-concept-axis!` (score in [-1, 1])
  - Facet gathering from tiles, structures, memories, agents
  - Vision radius (default 10 tiles, configurable)
  - Facet limit (default 16, max 64)
...
- ✅ ECS components extension (`backend/src/fantasia/sim/ecs/components.clj`)
  - Needs with water, health, mood + axes (hunger-axis, security-axis, rest-axis, warmth-axis, health-axis, mood-axis)
- ✅ Integration points
  - Death memory creation in `tick/mortality.clj`
  - Needs axes helper `query-need-axis!` in `agents.clj`
- ✅ Testing (34 tests in `spatial_facets_test.clj`)
  - Embedding system, entity registry, similarity, memory system, spatial queries, security axis
...
**🟡 BLOCKING ISSUE**: No frontend visualization
- Cannot observe memory creation/decay
- Cannot debug facet-based queries
- Cannot verify facet-based danger/safety drives behavior

**Next steps for completion**:
1. Create MemoryOverlay.tsx to visualize memories on map
2. Create FacetControls.tsx for facet limit configuration
3. Add WS op `config_facets` handler
4. Integrate facet queries into job priority logic
5. Test facet-based danger/safety drives agent behavior

**Progress**: ~14/18 SP complete (78%)
...
What we keep vs replace (review) #architecture #simulation #systems-design
----------------------------------------------
...
Epic dependency graph (high-level) #epic #project-management #planning
----------------------------------------
...
Sprint 1 — Hex map backbone + renderer #sprint #implementation #frontend #backend
----------------------------------------------
...
Sprint 3 — Colony core loop (jobs/items/needs) #sprint #colony-sim #simulation #ai #jobs #items
------------------------------------------------------
...
Sprint 4 — Champion control + Day/Night gate (Aker loop) #sprint #aker #champion #day-night #player-agency
----------------------------------------------------------------
...
Sprint 5 — 6 neighbor factions (ring settlements) #sprint #factions #diplomacy #settlements #politics
---------------------------------------------------------
...
Sprint 6 — 6 decks of cards + signs + collisions #sprint #cards #decks #signs #collision-resolution #game-mechanics
--------------------------------------------------------
...
How this plugs into your existing myth engine #myth #simulation #narrative #integration
===================================================
