      ...
      (let [new-qty (dec (:current-qty sp))
            ...
            world2 (assoc-in world1 [:agents agent-id :needs :food] 1.0)
             world3 (if (zero? new-qty)
                      ...
                      world2)]
         (log/log-info "[JOB:COMPLETE]" {:type :job/eat :agent-id agent-id :target pos :source "stockpile"})
         [world3 true])
      (if has-food-item?
        (let [items-at-tile (get items tile-key)
              ...
              new-items (cond
               ...
               world2 (assoc-in world1 [:agents agent-id :needs :food] 1.0)]
           (log/log-info "[JOB:COMPLETE]" {:type :job/eat :agent-id agent-id :target pos :source "food-item"})
           [world2 true])
        (if has-fruit-item?
          (let [items-at-tile (get items tile-key)
                ...
                new-items (cond
                 ...
                 world1 (assoc-in world [:items tile-key] new-items)
                 world2 (assoc-in world1 [:agents agent-id :needs :food] 1.0)]
             (log/log-info "[JOB:COMPLETE]" {:type :job/eat :agent-id agent-id :target pos :source "fruit-item"})
              [world2 true])
          (do
             (log/log-info "[JOB:COMPLETE]" {:type :job/eat :agent-id agent-id :target pos :source "none-found"})
             [world false]))))))
